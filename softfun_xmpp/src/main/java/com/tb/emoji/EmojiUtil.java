...
public class EmojiUtil {
        ...
        while (m.find()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (tempText.equals(emoji.getContent())) {
                    //转换为Span并设置Span的大小
                    sb.setSpan(new ImageSpan(
                                    context,
                                    decodeSampledBitmapFromResource(context.getResources(), emoji.getImageUri(), dip2px(context, 24), dip2px(context, 24))
                            ),
                            m.start(),
                            m.end(),
    ...
    public static SpannableStringBuilder getFaceText(Context context, String content){
        if(content!=null){
            SpannableStringBuilder sb = new SpannableStringBuilder(content);
            String regex = "\\[(\\S+?)\\]";
            Pattern p = Pattern.compile(regex);
            ...
            while (m.find()) {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (tempText.equals(emoji.getContent())) {
                        ...
                        sb.setSpan(new ImageSpan(
                                        context,
                                        decodeSampledBitmapFromResource(context.getResources(), emoji.getImageUri(), dip2px(context, 32), dip2px(context, 32))
                                ),
                                m.start(),
                                m.end(),
                                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            ...
            return SpannableStringBuilder.valueOf(content);
        }
    }
}
