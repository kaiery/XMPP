...
//                startActivity(item3);
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                ThreadUtils.runInThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        msg.setBody("");
                        jpe.setProperty(Const.MSGFLAG, Const.MSGFLAG_GROUP_DISMISS);//解散
                        jpe.setProperty(Const.GROUP_JID, groupname);
    ...
    public  void LeaveGroup(final String mTargetRoomJid){
        if(IMService.mMultiUserChatMap.containsKey(mTargetRoomJid+Const.ROOM_JID_SUFFIX)){
            ...
            JivePropertiesExtension jpe = new JivePropertiesExtension();
            msg.setBody("");
            jpe.setProperty(Const.MSGFLAG, Const.MSGFLAG_GROUP_LEAVE);
