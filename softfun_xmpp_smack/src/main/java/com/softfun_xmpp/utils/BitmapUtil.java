...
public final class BitmapUtil {
    ...
    public static Drawable byteToDrawable(byte[] bytes){
        if(bytes==null){
            return GlobalContext.getInstance().getResources().getDrawable(R.mipmap.ic_launcher);
        }
     ...
     * @return
     */
    public static Bitmap ScaleBitmapFixScreen(Bitmap bp){
        BitmapFactory.Options opts = new BitmapFactory.Options();// 解码图片的配置选项
        opts.inJustDecodeBounds = true;// 不去真实的解析bitmap,而是查询bitmap的宽高信息
        byte[] data = getBytesFromBitmap(bp);
        ...
        // 得到手机屏幕的宽高
        WindowManager wm = (WindowManager) GlobalContext.getInstance().getSystemService(Context.WINDOW_SERVICE);
        int screenWidth = wm.getDefaultDisplay().getWidth();
        int screenHeight = wm.getDefaultDisplay().getHeight();
        //System.out.println("屏幕宽度width:" + screenWidth);
        //System.out.println("屏幕高度height:" + screenHeight);
        float dx = width*1.0f / screenWidth*1.0f;
        float dy = height*1.0f / screenHeight*1.0f;
        ...
        //System.out.println("缩放后宽度："+bitmap.getWidth()+"  高度："+bitmap.getHeight());
        return bitmap;
    }
     ...
     * @return
     */
    public static Bitmap ScaleBitmap(Context context,int Rid,int w,int h){
        BitmapFactory.Options opts = new BitmapFactory.Options();// 解码图片的配置选项
        opts.inJustDecodeBounds = true;// 不去真实的解析bitmap,而是查询bitmap的宽高信息
        ...
        int scaleHeight = h;
        if(scaleWidth==0){
            scaleWidth = ((WindowManager)context.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay().getWidth();
        }
        if(scaleHeight==0){
            scaleHeight = ((WindowManager)context.getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay().getHeight();
        }
        int dx = width / scaleWidth;
        int dy = height / scaleHeight;
        int scale = 1;
        if (dx > dy && dy > 1) {
            scale = dx;
        }
        ...
        opts.inJustDecodeBounds = false;// 真实的解析bitmap
        Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), Rid, opts);
        return bitmap;
    }
    /**
     * 图片压缩处理（使用Options的方法）
    ...
    public static Bitmap decodeSampledBitmapFromResource(Resources res, int resId,
        ...
        options.inSampleSize = calculateInSampleSize_int(options, reqWidth, reqHeight);
        // 使用获取到的inSampleSize值再次解析图片
        options.inJustDecodeBounds = false;
        return BitmapFactory.decodeResource(res, resId, options);
    }
    public static int calculateInSampleSize_int(BitmapFactory.Options options,
                                            int reqWidth, int reqHeight) {
        // 源图片的高度和宽度
        final int height = options.outHeight;
        final int width = options.outWidth;
    ...
    public static Bitmap blur(Context context, Bitmap sentBitmap, int radius) {
        ...
        if (radius < 0) {
            radius = 0;
            if (DEBUG) {
                System.out.println(TAG + "radius must be 0 < r < 25 , forcing radius=0");
            }
        } else if (radius > 25) {
            radius = 25;
            if (DEBUG) {
                System.out.println(TAG+ "radius must be 0 < r < 25 , forcing radius=25");
            }
        }
